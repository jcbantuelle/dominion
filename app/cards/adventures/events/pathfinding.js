Pathfinding = class Pathfinding extends Event {
  ...
  buy(game, player_cards) {
    let eligible_piles = _.filter(game.cards, function(card) {
      let has_player_token = _.some(card.tokens, function(token) {
        return token.name === 'card' && token.username === player_cards.username
      })
    ...
    turn_event_processor.process(Pathfinding.place_token)
  }

  static place_token(game, player_cards, response) {
    let token_placer = new TokenPlacer(game, player_cards, 'card', response[0])
    token_placer.place()
  }
}
