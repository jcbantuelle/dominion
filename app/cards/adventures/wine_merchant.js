WineMerchant = class WineMerchant extends Card {
  ...
  play(game, player_cards) {
    ...
    this.move_to_tavern(game, player_cards, 'Wine Merchant')
  }
  ...
  end_buy_event(game, player_cards) {
    let turn_event_id = TurnEventModel.insert({
      ...
      type: 'choose_yes_no',
      instructions: `Discard ${CardView.render(this)}?`,
      minimum: 1,
      maximum: 1
    })
  ...
  static discard_card(game, player_cards, response) {
    if (response === 'yes') {
      let card_discarder = new CardDiscarder(game, player_cards, 'tavern', 'Wine Merchant')
