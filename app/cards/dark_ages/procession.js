Procession = class Procession extends Card {
  ...
  static play_twice(game, player_cards, selected_cards) {
    ...
    if (played_card_index !== -1) {
      let card_trasher = new CardTrasher(game, player_cards, 'playing', selected_card.name)
      card_trasher.trash()
    }
    ...
    let eligible_cards = _.filter(game.cards, function(card) {
      ...
      return card.count > 0 && card.top_card.purchasable && game_card_coin_cost === (selected_card_coin_cost + 1) && card.top_card.potion_cost === selected_card.potion_cost
    })
