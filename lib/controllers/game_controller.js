GameController = LoggedInController.extend({
  ...
  data: function () {
    ...
    if (game) {
      ...
      let pending_players = TurnEvents.find().fetch()

      return {
        game: game,
        player_cards: player_cards,
        turn_event: turn_event,
        pending_players: _.compact(_.map(TurnEvents.find().fetch(), function(turn_event) {
          return turn_event.username ? `<strong>${turn_event.username}</strong>` : undefined
        })).join(' and '),
